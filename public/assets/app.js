...
document.addEventListener('keydown',event=>{
  ...
  if(visible('reportModal'))return closeDecisionReport();
  if(visible('versionModal'))return closeVersionCompare();
  if(visible('goalModal'))return closeGoalModal();
  if(visible('profileModal'))return closeProfile();
});

renderHome();renderAskExamples();navigate('home');refreshRuntimeCapabilities().then(async()=>{await syncAccountIfAvailable();if(state.route==='dashboard')renderDashboard();});
